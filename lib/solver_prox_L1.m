...
if ~isfield(param, 'minmax'), param.minmax = 0; end
if ~isfield(param, 'real'), param.real = 0; end
if ~isfield(param, 'bound_mod'), param.bound_mod = Inf; end
if ~isfield(param, 'support_im'), param.support_im = ones(size(x)) ; end

% Useful functions
soft = @(z, T) sign(z).*max(abs(z)-T, 0);

% Projection
if param.tight && ~param.pos && ~param.real && ~param.minmax && ~param.support_im && ~param.bound_mod  % TIGHT FRAME CASE
...
sol = real(sol); 
sol(sol<0) = 0 ;
%sol(sol<param.xmin) = param.xmin; sol(sol>param.xmax) = param.xmax;
% % % sol(~param.support_im) =0 ;
% % % sol(abs(sol)>param.bound_mod) = param.bound_mod * ( sol(abs(sol)>param.bound_mod)./ abs(sol(abs(sol)>param.bound_mod)) ) ;
end

% % % if param.minmax
% % % % % % sol = real(sol); %sol(sol<param.xmin) = param.xmin; sol(sol>param.xmax) = param.xmax;
% % % sol(~param.support_im) =0 ;
% % % if ~isreal(x)
% % % sr = real(sol) ; si = imag(sol) ;
% % % sr(sr<param.xmin) = param.xmin ; si(si<param.xmin) = param.xmin ;
% % % sr(sr>param.xmax) = param.xmax ; si(si>param.xmax) = param.xmax ;
% % % sol = sr + 1i* si ;
% % % else
% % % sol(sol<param.xmin) = param.xmin ;
% % % sol(sol>param.xmax) = param.xmax ;
% % % end
% % % end
